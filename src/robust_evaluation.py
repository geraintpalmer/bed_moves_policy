...
if __name__ == '__main__':
    ...
    parser.add_argument('stage_to_test', help='The stage with the best performing policy.')
    ...
    stage_to_test = int(args.stage_to_test)
    ...
    stage_param_name = f"epsilon_{round(epsilon_to_test, 3)}"

    keys = np.memmap(f"{args.experiment}/results/stage_{stage_to_test}_overall_keys_{stage_param_name}.bin", dtype=np.int64, mode='r')
    qvals = np.memmap(f"{args.experiment}/results/stage_{stage_to_test}_overall_qvals_{stage_param_name}.bin", dtype=np.float32, mode='r')
    policy_keys, policy_actions = rl.initialise_policy(
        keys_array=keys,
        qval_array=qvals
    )
    policy_keys_path = f"{args.experiment}/results/stage_{stage_to_test}_overall_policykeys_{stage_param_name}.bin"
    policy_keys.tofile(policy_keys_path)
    policy_actions_path = f"{args.experiment}/results/stage_{stage_to_test}_overall_policyactions_{stage_param_name}.bin"
    ...
    df.to_csv(f"{args.experiment}/results/robust_evaluation.csv", index=False)

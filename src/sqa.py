...
def get_state_probabilities(seed, max_time, warmup):
    ...
    N = ciw.create_network(
        arrival_distributions={
            'Stage 2': [ciw.dists.Exponential(rate=2.8), None, None],
            'Stage 3': [ciw.dists.Exponential(rate=1.9), None, None],
        ...
        number_of_servers=[float('inf'), 16, 16],
        routing={
    ...
    prob_less_than = {i: 0.0 for i in range(17)}
    prob_equal = {i: 0.0 for i in range(17)}
    prob_total = {i: 0.0 for i in range(17)}
    for n1 in range(17):
        for n2 in range(17):
    ...
    p = [prob_less_than[n] + (prob_equal[n] / 2) for n in range(17)]
    return np.array(p)
...
ax.bar(range(17), p, color='darkorange', edgecolor='black')
